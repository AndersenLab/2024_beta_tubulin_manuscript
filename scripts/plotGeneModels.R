...
plotNormGeneFeatures <- function(sub_gff,cust) {
  ...
  if (cust == T) {
    groups <- data.frame(mAlias=c("tbb-1","tbb-2","mec-7","tbb-4","ben-1","tbb-6"),ngroup=c(6,5,4,3,2,1))
    GFFbyL2 <- GFFbyL2 %>%
      dplyr::select(-ngroup) %>%
...
dels <- readr::read_tsv("/projects/b1059/projects/Nicolas/collabs/forJB/geneModels_ben1/tbb-del.tsv")
...
gene <-"WBGene00006538"
features5 <- getGeneFeature(path,gene,dels)

gene <-"WBGene00006539"
features6 <- getGeneFeature(path,gene,dels)
...
sub_gff <- rbind(features[[1]] %>% dplyr::mutate(tag="ben-1") %>% dplyr::mutate(spp="N2"),
                 features2[[1]] %>% dplyr::mutate(tag="tbb-1") %>% dplyr::mutate(spp="N2"),
                 features3[[1]] %>% dplyr::mutate(tag="tbb-2") %>% dplyr::mutate(spp="N2"),
                 features4[[1]] %>% dplyr::mutate(tag="mec-7") %>% dplyr::mutate(spp="N2"),
                 features5[[1]] %>% dplyr::mutate(tag="tbb-4") %>% dplyr::mutate(spp="N2"),
                 features6[[1]] %>% dplyr::mutate(tag="tbb-6") %>% dplyr::mutate(spp="N2"))

p1 <- plotNormGeneFeatures(sub_gff,T)

ggsave(plot = p1,"/projects/b1059/projects/Nicolas/collabs/forJB/geneModels_ben1/tbb_genes_wDel.tiff",device = 'tiff',dpi = 600,width = 4,height = 3.5 ,units = 'in')
